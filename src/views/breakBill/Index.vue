<template>
	<div class="inteledi-breakBill">
		<w-header title="报损单" :border="true">
			<headEditBtn
				v-show="$fxAuth('breakBill.check')"
				slot="header-right"
		...
		></filterTab>
		<div class="breakBill-content">
			<van-pull-refresh v-model="pullRefreshLoading" v-show="orderList.length !== 0" class="breakBill-list" @refresh="onPullRefresh">
				<van-list
					v-model="listLoading"
		...
		<div class="breakBill-bottom">
			<van-button
				v-show="!orderEditFlag && $fxAuth('breakBill.add')"
				class="add-btn fx-no-radius"
				type="primary"
				@click="onCreateClick"
			>添加单据</van-button>
			<van-button
				v-show="orderEditFlag && $fxAuth('breakBill.check')"
				class="add-btn fx-no-radius"
				type="primary"
...
import orderBlock from '@/components/breakBill/orderList/OrderBlock.vue'
import filterPop from '@/components/breakBill/orderList/FilterPop.vue'
import filterTab from '@/components/breakBill/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	name: 'breakBill',
	components: {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/breakBillDetails']
	...
	methods: {
		orderListPostHandler (params) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/breakBills/users/${this.loginerId}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		onFilterConfirm (filterParams) {
			...
			const params = {
				...
				reckonBreak: filterParams.reckonBreak === '' ? null : filterParams.reckonBreak
			}
			this.orderListPostHandler(params).then(res => {
				this.orderList = res.storeBills
			})
		...
		onCreateClick () {
			this.$router.push('createBreakBill')
		},
		onOrderEditClick () {
