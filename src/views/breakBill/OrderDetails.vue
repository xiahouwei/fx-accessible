<template>
	<div class="inteledi-breakBill-details">
		...
		<orderItem
			...
			:autoType="setAutoType(orderInfoData)"
		...
		></orderItem>
		<div class="breakBill-bottom">
			<van-button
				v-show="isBillState(0) && $fxAuth('breakBill.del')"
				class="bottom-btn fx-no-radius"
				type="danger"
				@click="onClickHandler('onDeleteClick')"
			>删除</van-button>
			<van-button
				v-show="isBillState(0) && $fxAuth('breakBill.check')"
				class="bottom-btn fx-no-radius"
				type="info"
				@click="onClickHandler('onExpressClick')"
			>审核</van-button>
			<van-button
				v-show="isBillState(1) && $fxAuth('breakBill.unCheck')"
				class="bottom-btn fx-no-radius"
				type="danger"
				@click="onClickHandler('onUnExpressClick')"
			>反审</van-button>
			<van-button
				v-show="isBillState(0) && $fxAuth('breakBill.save')"
				class="bottom-btn fx-no-radius"
				type="primary"
...
import orderInfo from '@/components/breakBill/createOrder/OrderInfo.vue'
import orderItem from '@/components/breakBill/createOrder/OrderItem.vue'
import listComp from '@/components/common/ListComp.vue'
import storeItemComp from '@/components/breakBill/common/StoreItemComp.vue'
import itemRowEditPop from '@/components/breakBill/common/ItemRowEditPop.vue'
import itemSearchPop from '@/components/breakBill/common/itemSearchPop.vue'
import itemEditComp from '@/components/breakBill/common/itemEditComp.vue'
import storeBomComp from '@/components/breakBill/common/StoreBomComp.vue'
import bomEditComp from '@/components/breakBill/common/bomEditComp.vue'
import bomSearchPop from '@/components/breakBill/common/bomSearchPop.vue'
const ITEM_SIZE = 50
export default {
	...
	beforeRouteLeave (to, from, next) {
		...
		if (to.path === '/breakBill' && !this.orderDeleteFlag && this.orderInfoData.billState === 0) {
			this.$dialog.confirm({
				title: '提示',
	...
	methods: {
		setAutoType () {
			let detailsLength = this.orderItemList.reduce((pre, cur) => {
				let arr = cur.details.filter(item => {
					return item.changeType !== 'del'
				})
				return pre + arr.length
			}, 0)
			if (detailsLength === 0 && this.orderInfoData.autoType === 3) {
				this.orderInfoData.autoType = 1
			}
			return this.orderInfoData.autoType
		},
		createOrderDetails () {
		...
		getOrderDetails () {
			return new Promise(resolve => {
				this.$fxHttp.requestGet({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/breakBills/${this.query.orderId}`
				}).then(res => {
					resolve(res)
				})
			})
		...
		expressHandler (params) {
			this.$fxHttp.requestPut({
				url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/breakBills/${this.query.orderId}/users/${this.loginerId}`,
				data: params,
				query: {
					m: 'editAndCheck'
				}
		...
		getBomItem (selectItems) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.orderInfoData.shopDTO.id}/breakBills/details`,
					data: params,
					query: {
						m: 'breakBillBom'
					}
