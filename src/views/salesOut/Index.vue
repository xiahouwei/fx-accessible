<template>
	<div class="inteledi-salesOut">
		<w-header title="外销出库单" :border="true">
			<headEditBtn
				v-show="$fxAuth('salesOutBill.check')"
				slot="header-right"
		...
		></filterTab>
		<div class="salesOut-content">
			<van-pull-refresh v-model="pullRefreshLoading" v-show="orderList.length !== 0" class="salesOut-list" @refresh="onPullRefresh">
				<van-list
					v-model="listLoading"
		...
		<div class="salesOut-bottom">
			<van-button
				v-show="!orderEditFlag && $fxAuth('salesOutBill.add')"
				class="add-btn fx-no-radius"
				type="primary"
				@click="onCreateClick"
			>添加单据</van-button>
			<van-button
				v-show="orderEditFlag && $fxAuth('salesOutBill.check')"
				class="add-btn fx-no-radius"
				type="primary"
...
import orderBlock from '@/components/salesOut/orderList/OrderBlock.vue'
import filterPop from '@/components/salesOut/orderList/FilterPop.vue'
import filterTab from '@/components/salesOut/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	name: 'salesOut',
	components: {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/salesOutDetails']
	...
	methods: {
		orderListPostHandler (params) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/salesout/users/${this.loginerId}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		onFilterConfirm (filterParams) {
			...
			const params = {
				...
				billState: this.filterState,
				reCheckState: this.filterBillReState,
				customerId: filterParams.customerId === '' ? null : filterParams.customerId,
				shopId: filterParams.inShopId === '' ? null : filterParams.inShopId,
		...
		onCreateClick () {
			this.$router.push('createSalesOut')
		},
		onOrderEditClick () {
