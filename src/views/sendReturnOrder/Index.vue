<template>
	<div class="inteledi-shopOrder">
		<w-header title="直送退货单" :border="true">
			<headEditBtn
				v-show="$fxAuth('shopDirectRtnBill.check')"
				slot="header-right"
		...
		<filterTab
			:state="computedState"
			:sendBillState="computedSendBIllState"
			@on-state-click="onStateClick"
			@on-sendBillState-click="onSendBillStateClick"
		></filterTab>
		...
		<div class="shopOrder-bottom">
			<van-button
				v-show="!orderEditFlag && $fxAuth('shopDirectRtnBill.add')"
				class="add-btn fx-no-radius"
				type="primary"
				@click="onCreateClick"
			>添加单据</van-button>
			<van-button
				v-show="orderEditFlag && $fxAuth('shopDirectRtnBill.check')"
				class="add-btn fx-no-radius"
				type="primary"
		...
		/>
		<van-actionsheet
			v-model="sendBillStateActionShow"
			:actions="sendBillStateActions"
			cancel-text="取消"
			@select="onSendBillSateSelect"
		/>
...
import orderBlock from '@/components/sendReturnOrder/orderList/OrderBlock.vue'
import filterPop from '@/components/sendReturnOrder/orderList/FilterPop.vue'
import filterTab from '@/components/sendReturnOrder/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	...
	data () {
		return {
			...
			filterSendBillState: null,
			sendBillStateActionShow: false,
			sendBillStateActions: [
				{ id: null, name: '全部' },
				{ id: '9', name: '已退回' },
				{ id: '2', name: '已同意' },
				{ id: '1', name: '已发送' },
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/sendReturnOrderDetails']
	...
	computed: {
		...
		computedSendBIllState () {
			return this.sendBillStateActions.find(item => {
				return item.id === this.filterSendBillState
			}).name
		},
	...
	methods: {
		orderListPostHandler (params) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/directrtnbills/users/${this.loginerId}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		refreshOrderList () {
			return new Promise(resolve => {
				...
				let params = {
					...
					billSendState: this.filterSendBillState
				}
		...
		onSendBillStateClick () {
			this.sendBillStateActionShow = true
		},
		onSendBillSateSelect (item) {
			this.filterSendBillState = item.id
			this.sendBillStateActionShow = false
			this.refreshOrderList()
		},
		onClickOrder (item) {
			this.$router.push({
		...
		onCreateClick () {
			this.$router.push('createSendReturnOrder')
		},
		onOrderEditClick () {
