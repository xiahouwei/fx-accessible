<template>
	<w-app-page>
		<w-header title="新增中心采购入库单" :border="true"></w-header>
		<orderInfo
			ref="orderInfo"
			:data="orderInfoData"
...
export default {
	...
	beforeRouteLeave (to, from, next) {
		...
		if (to.path === '/stockInBill4cld' && this.orderItemList.length !== 0 && !this.backAbsolute) {
			this.$dialog.confirm({
				title: '提示',
	...
	methods: {
		...
		getBillOtherAuth () {
			return this.$fxApi('stockInBill4cld.getBillOtherAuth', '0').then(res => {
				this.$store.commit({
					type: 'EXTEND_AUTH',
		...
		createHandler (params) {
			this.$fxApi('stockInBill4cld.createOrder')({ data: params }).then(res => {
		...
		getSaveParams () {
			...
			addItems.noTransMoney = addItems.inMoney
			addItems.noTransInTaxesMoney = addItems.inMoneyWithTax
