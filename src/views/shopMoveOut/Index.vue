<template>
	<div class="inteledi-shopMoveOut">
		<w-header title="调拨出库单" :border="true">
			<headEditBtn
				v-show="$fxAuth('shopMoveOutBill.check')"
				slot="header-right"
		...
		></filterTab>
		<div class="shopMoveOut-content">
			<van-pull-refresh v-model="pullRefreshLoading" v-show="orderList.length !== 0" class="shopMoveOut-list" @refresh="onPullRefresh">
				<van-list
					v-model="listLoading"
		...
		<div class="shopMoveOut-bottom">
			<van-button
				v-show="orderEditFlag && $fxAuth('shopMoveOutBill.check')"
...
import orderBlock from '@/components/shopMoveOut/orderList/OrderBlock.vue'
import filterPop from '@/components/shopMoveOut/orderList/FilterPop.vue'
import filterTab from '@/components/shopMoveOut/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	name: 'shopMoveOut',
	components: {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/shopMoveOutDetails']
	...
	methods: {
		orderListPostHandler (params) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/shopMoveOutBills/users/${this.loginerId}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		onFilterConfirm (filterParams) {
			...
			const params = {
				...
				outHouseId: filterParams.outStoreId === '' ? null : filterParams.outStoreId,
