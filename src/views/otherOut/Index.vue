<template>
	<div class="inteledi-otherOut">
		<w-header title="其他出库单" :border="true">
			<headEditBtn
				v-show="$fxAuth('otherOutBill.check')"
				slot="header-right"
		...
		></filterTab>
		<div class="otherOut-content">
			<van-pull-refresh v-model="pullRefreshLoading" v-show="orderList.length !== 0" class="otherOut-list" @refresh="onPullRefresh">
				<van-list
					v-model="listLoading"
		...
		<div class="otherOut-bottom">
			<van-button
				v-show="!orderEditFlag && $fxAuth('otherOutBill.add')"
				class="add-btn fx-no-radius"
				type="primary"
				@click="onCreateClick"
			>添加单据</van-button>
			<van-button
				v-show="orderEditFlag && $fxAuth('otherOutBill.check')"
				class="add-btn fx-no-radius"
				type="primary"
...
import orderBlock from '@/components/otherOut/orderList/OrderBlock.vue'
import filterPop from '@/components/otherOut/orderList/FilterPop.vue'
import filterTab from '@/components/otherOut/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	name: 'otherOut',
	components: {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/otherOutDetails']
	...
	methods: {
		orderListPostHandler (params) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/otherOutBills/users/${this.loginerId}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		onCreateClick () {
			this.$router.push('createOtherOut')
		},
		onOrderEditClick () {
