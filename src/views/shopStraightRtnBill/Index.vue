<template>
	<div class="inteledi-shopStraightRtnBill">
		<w-header title="门店直通退货单" :border="true">
			<headEditBtn
				v-show="$fxAuth('shopStraightRtnBill.check')"
				slot="header-right"
		...
		></filterTab>
		<div class="shopStraightRtnBill-content">
			<van-pull-refresh v-model="pullRefreshLoading" v-show="orderList.length !== 0" class="shopStraightRtnBill-list" @refresh="onPullRefresh">
				<van-list
					v-model="listLoading"
		...
		<div class="shopStraightRtnBill-bottom">
			<van-button
				v-show="!orderEditFlag && $fxAuth('shopStraightRtnBill.add')"
				class="add-btn fx-no-radius"
				type="primary"
				@click="onCreateClick"
			>添加单据</van-button>
			<van-button
				v-show="orderEditFlag && $fxAuth('shopStraightRtnBill.check')"
				class="add-btn fx-no-radius"
				type="primary"
...
import orderBlock from '@/components/shopStraightRtnBill/orderList/OrderBlock.vue'
import filterPop from '@/components/shopStraightRtnBill/orderList/FilterPop.vue'
import filterTab from '@/components/shopStraightRtnBill/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/shopStraightRtnBillDetails']
	...
	methods: {
		orderListPostHandler (params) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/shopStraightRtn/users/${this.loginerId}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		onCreateClick () {
			this.$router.push('createShopStraightRtnBill')
		},
		onOrderEditClick () {
...
.inteledi-shopStraightRtnBill {
	height:100%
	background:$fxWhite
	display: flex
	flex-direction: column
	.shopStraightRtnBill-content {
		flex:1
		overflow:scroll
		.shopStraightRtnBill-list {
			min-height: 526px
		}
	}
