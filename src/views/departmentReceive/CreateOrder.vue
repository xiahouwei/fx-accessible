<template>
	<div class="inteledi-departmentReceive-create">
		<w-header title="新增部门领用单" :border="true"></w-header>
		<orderInfo
			ref="orderInfo"
			:data="orderInfoData"
		...
		>
			<itemSearchPop
				...
				:isDisabled="!$fxAuth('departmentReceiveBill.save')"
	...
	methods: {
		...
		createHandler (params) {
			this.$fxHttp.requestPost({
				url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/departmentReceive`,
				data: params
			}).then(res => {
...
.inteledi-departmentReceive-create {
	height:100%
	background:$fxWhite
	display: flex
