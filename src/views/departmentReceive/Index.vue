<template>
	<div class="inteledi-departmentReceive">
		<w-header title="部门领用单" :border="true">
			<headEditBtn
				v-show="$fxAuth('departmentReceiveBill.check')"
				slot="header-right"
		...
		></filterTab>
		<div class="departmentReceive-content">
			<van-pull-refresh v-model="pullRefreshLoading" v-show="orderList.length !== 0" class="departmentReceive-list" @refresh="onPullRefresh">
				<van-list
					v-model="listLoading"
		...
		<div class="departmentReceive-bottom">
			<van-button
				v-show="!orderEditFlag && $fxAuth('departmentReceiveBill.add')"
				class="add-btn fx-no-radius"
				type="primary"
				@click="onCreateClick"
			>添加单据</van-button>
			<van-button
				v-show="orderEditFlag && $fxAuth('departmentReceiveBill.check')"
				class="add-btn fx-no-radius"
				type="primary"
...
import orderBlock from '@/components/departmentReceive/orderList/OrderBlock.vue'
import filterPop from '@/components/departmentReceive/orderList/FilterPop.vue'
import filterTab from '@/components/departmentReceive/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/departmentReceiveDetails']
	...
	methods: {
		orderListPostHandler (params) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/departmentReceive/users/${this.loginerId}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		onFilterConfirm (filterParams) {
			...
			const params = {
				...
				departmentId: filterParams.departmentId === '' ? null : filterParams.departmentId,
				outHouseId: filterParams.storeId === '' ? null : filterParams.storeId,
		...
		onCreateClick () {
			this.$router.push('createDepartmentReceive')
		},
		onOrderEditClick () {
...
.inteledi-departmentReceive {
	height:100%
	background:$fxWhite
	display: flex
	flex-direction: column
	.departmentReceive-content {
		flex:1
		overflow:scroll
		.departmentReceive-list {
			min-height: 526px
		}
	}
