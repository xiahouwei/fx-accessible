<template>
	<div class="inteledi-uniformDistributionRtn-create">
		<w-header title="新增统配退货单" :border="true"></w-header>
		<orderInfo
			ref="orderInfo"
			:data="orderInfoData"
		...
		>
			<itemSearchPop
				...
				:data="searchItem"
				:isDisabled="!$fxAuth('uniformDistributionRtnBill.save')"
...
export default {
	...
	beforeRouteLeave (to, from, next) {
		...
		if (to.path === '/uniformDistributionRtnBill') {
			this.$dialog.confirm({
				title: '提示',
	...
	methods: {
		...
		createHandler (params) {
			this.$fxHttp.requestPost({
				url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/distributionRtn`,
				data: params
			}).then(res => {
...
.inteledi-uniformDistributionRtn-create {
	height:100%
	background:$fxWhite
	display: flex
