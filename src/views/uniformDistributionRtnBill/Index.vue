<template>
	<div class="inteledi-uniformDistributionRtn">
		<w-header title="统配退货单" :border="true">
			<headEditBtn
				v-show="$fxAuth('uniformDistributionRtnBill.check')"
				slot="header-right"
		...
		></filterTab>
		<div class="uniformDistributionRtn-content">
			<van-pull-refresh v-model="pullRefreshLoading" v-show="orderList.length !== 0" class="uniformDistributionRtn-list" @refresh="onPullRefresh">
				<van-list
					v-model="listLoading"
		...
		<div class="uniformDistributionRtn-bottom">
			<van-button
				v-show="!orderEditFlag && $fxAuth('uniformDistributionRtnBill.add')"
				class="add-btn fx-no-radius"
				type="primary"
				@click="onCreateClick"
			>添加单据</van-button>
			<van-button
				v-show="orderEditFlag && $fxAuth('uniformDistributionRtnBill.check')"
				class="add-btn fx-no-radius"
				type="primary"
...
import orderBlock from '@/components/uniformDistributionRtnBill/orderList/OrderBlock.vue'
import filterPop from '@/components/uniformDistributionRtnBill/orderList/FilterPop.vue'
import filterTab from '@/components/uniformDistributionRtnBill/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/uniformDistributionRtnBillDetails']
	...
	methods: {
		...
		onCreateClick () {
			this.$router.push('createUniformDistributionRtnBill')
		},
		onOrderEditClick () {
...
.inteledi-uniformDistributionRtn {
	height:100%
	background:$fxWhite
	display: flex
	flex-direction: column
	.uniformDistributionRtn-content {
		flex:1
		overflow:scroll
		.uniformDistributionRtn-list {
			min-height: 526px
		}
	}
