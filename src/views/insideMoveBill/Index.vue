<template>
	<div class="inteledi-insideMoveBill">
		<w-header title="内部调拨单" :border="true">
			<headEditBtn
				v-show="$fxAuth('insideMoveBill.check')"
				slot="header-right"
		...
		></filterTab>
		<div class="insideMoveBill-content">
			<van-pull-refresh v-model="pullRefreshLoading" v-show="orderList.length !== 0" class="insideMoveBill-list" @refresh="onPullRefresh">
				<van-list
					v-model="listLoading"
		...
		<div class="insideMoveBill-bottom">
			<van-button
				v-show="!orderEditFlag && $fxAuth('insideMoveBill.add')"
				class="add-btn fx-no-radius"
				type="primary"
				@click="onCreateClick"
			>添加单据</van-button>
			<van-button
				v-show="orderEditFlag && $fxAuth('insideMoveBill.check')"
				class="add-btn fx-no-radius"
				type="primary"
...
import orderBlock from '@/components/insideMoveBill/orderList/OrderBlock.vue'
import filterPop from '@/components/insideMoveBill/orderList/FilterPop.vue'
import filterTab from '@/components/insideMoveBill/orderList/FilterTab.vue'
const LIST_SIZE = 50
export default {
	name: 'insideMoveBill',
	components: {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/insideMoveBillDetails']
	...
	methods: {
		orderListPostHandler (params) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/insideMoveBill/users/${this.loginerId}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		onCreateClick () {
			this.$router.push('createInsideMoveBill')
		},
		onOrderEditClick () {
		...
		restRouterParams (params) {
			if (!this.$fxUtils.isEmptyObjSHW(params)) {
				...
				this.filterReCheckState = params.reCheckState ? params.reCheckState : null
			}
		}
	}
}
...
.inteledi-insideMoveBill {
	height:100%
	background:$fxWhite
	display: flex
	flex-direction: column
	.insideMoveBill-content {
		flex:1
		overflow:scroll
		.insideMoveBill-list {
			min-height: 526px
