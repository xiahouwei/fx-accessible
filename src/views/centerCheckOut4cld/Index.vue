...
import orderBlock from '@/components/centerCheckOut4cld/orderList/OrderBlock.vue'
import filterPop from '@/components/centerCheckOut4cld/orderList/FilterPop.vue'
const LIST_SIZE = 50
export default {
	...
	beforeRouteEnter (to, from, next) {
		...
		const refreshPath = ['/centerCheckOut4cldDetails', '/centerCheckBill4cldDetails']
	...
	methods: {
		orderListPostHandler (params) {
			...
			return this.$fxApi('centerCheckOut4cld.getOrderListData')({ data: params })
		},
		refreshOrderList () {
		...
		onFilterConfirm (filterParams) {
			...
			const params = {
				...
				billStateFlag: this.filterStateAndRestate,
				weightState: this.filterSortingState,
				sendOrganId: filterParams.rdcId === '' ? null : filterParams.rdcId,
