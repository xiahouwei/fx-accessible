	...
	<div class="filter-popup">
		...
		<div ref="filterContent" class="filter-content">
			<van-cell-group>
				...
				<van-cell title="机构" :value="shop" value-class="info-item-value" is-link @click="onShopClick" />
				<van-cell title="出库仓库" :value="store" value-class="info-item-value" is-link @click="onStoreClick" />
				<van-cell title="部门" :value="department" value-class="info-item-value" is-link @click="onDepartmentClick" />
		...
		<van-popup v-model="departmentPopShow" position="right" class="fx-list-popup-300" :lock-scroll="false">
			<listComp
				:visibleFlag.sync="departmentPopShow"
				:data="departmentList"
				:loadFinished="departmentListLoadFinished"
				:loading.sync="onDepartmentLoading"
				title="部门检索"
				@on-search="searchDepartment"
				@on-select="selectDepartment"
				@on-load="onDepartmentLoad"
			></listComp>
		</van-popup>
...
export default {
	...
	data () {
		return {
			...
			departmentListStart: 0,
			departmentListLoadFinished: false,
			onDepartmentLoading: false,
			searchDepartmentKey: null,
			itemPopShow: false,
			storeList: [],
			departmentList: [],
			itemList: [],
	...
	methods: {
		...
		// {{{部门
		onDepartmentClick () {
			this.departmentListStart = 0
			this.searchDepartmentKey = null
			this.departmentListLoadFinished = false
			this.onDepartmentRefresh()
		},
		searchDepartment (keyword) {
			this.departmentListStart = 0
			this.searchDepartmentKey = keyword
			this.onDepartmentRefresh()
		},
		...
		onDepartmentRefresh () {
			this.getDepartmentListData().then(res => {
				this.onDepartmentLoading = false
				this.departmentList = res.departmentDTOData
		...
		onDepartmentLoad () {
			this.getDepartmentListData().then(res => {
				this.onDepartmentLoading = false
				this.departmentList.push(...res.departmentDTOData)
			})
		},
		getDepartmentListData () {
			return new Promise(resolve => {
				let params = {
					start: this.departmentListStart,
					size: ITEM_SIZE,
					keyword: this.searchDepartmentKey
				}
				if (this.shopId) {
				...
				this.$fxHttp.requestGet({
					url: `${this.$fxBaseApi.url}/accounts/${this.loginerShopId}/users/${this.loginerId}/department/report`,
					data: params
				}).then(res => {
					this.departmentListStart += ITEM_SIZE
					if (res.departmentDTOData.length < ITEM_SIZE) {
						this.departmentListLoadFinished = true
					} else {
						this.departmentListLoadFinished = false
					}
					resolve(res)
