<template>
	<div class="order-item-block fx-shadow-block">
		<div class="item-title">
			...
			<i v-show="!isDisabled && isAutoType([1]) && $fxAuth('departmentReceiveBill.departmentBOM') && $fxAuth('departmentReceiveBill.editAmount')" class="title-selectbom-icon title-icon fl-r" @click="onSelectBomClick"></i>
			<i v-show="$fxAuth('departmentReceiveBill.departmentBOM') && isAutoType([1])" class="title-viewbom-icon title-icon fl-r" @click="onViewBomClick"></i>
			<i v-show="!isDisabled && isAutoType([1]) && $fxAuth('departmentReceiveBill.editAmount')" class="title-add-icon title-icon fl-r" @click="onAddClick"></i>
	...
	methods: {
		...
		onDeleteClick (item, index, details) {
			if (item.id) {
				...
				this.$set(item, 'busUnitOutAmount', 0)
			} else {
