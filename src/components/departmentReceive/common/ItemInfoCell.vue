<template>
	<div class="item-info-cell">
		...
		<div class="item-details">
			...
			<div class="details-cell">
				<div v-if="isDisabled || (itemData.isWeightSign && !sysDZCCanModifyAcount) || !$fxAuth('departmentReceiveBill.editAmount')" class="item-amount">
				...
				<div v-else class="item-amount-edit">
					...
					<div class="unit-text">
						...
						<van-icon class="unit-icon" v-show="!isDisabled && isAutoType([1])" name="arrow-down" />
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		calcMoney (target, amountOrMoney, itemData) {
			let { busUnitOutAmount, price, taxPrice, tax } = itemData
			let money = 0
			let moneyWithTax = 0
			let itemPrice = 0
			let itemTaxPrice = 0
		...
		onUnitClick () {
			if (this.isDisabled || this.autoType === 5) {
				return false
			}
