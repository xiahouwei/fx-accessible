	...
	<div class="itemRowEdit-popup">
		...
		<div ref="rowEditContent" class="itemRowEdit-content">
			<van-cell-group>
				...
				<van-cell
					v-show="$fxAuth('shopDirectInBill.seePrice')"
					title="非税单价"
					:value="currentItemData.price"
					value-class="info-item-value"
				/>
				<van-cell
					v-show="$fxAuth('shopDirectInBill.seeNoTaxMoney')"
					title="非税金额"
					:value="currentItemData.inMoney"
					value-class="info-item-value"
				/>
				<van-cell
					v-show="$fxAuth('shopDirectInBill.seeTax')"
					title="税率"
				...
				/>
				<van-cell
					v-show="$fxAuth('shopDirectInBill.seeTaxMoney')"
					title="税额"
					:value="currentItemData.taxInMoney"
					value-class="info-item-value"
				/>
				<van-pro-number-field
					...
					:disabled="isDisabled || !$fxAuth('shopDirectInBill.editPrice') || busUnitInAmountIs0"
...
export default {
	...
	methods: {
		...
		onTaxClick () {
			...
			if (!this.$fxAuth('shopDirectInBill.ediTax')) {
				this.$fxToast({
					message: '当前没有修改税率的权限!',
					position: 'top'
				})
