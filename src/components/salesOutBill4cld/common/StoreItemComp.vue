<template>
	<w-app-page class="store-item">
		<w-header title="编辑中心外销出库单明细" :leftOptions="leftOptions" @on-click-back="onCloseClick"></w-header>
		<div class="add-store-bar">
			<van-button class="add-store-btn" type="primary" round @click="onHouseRefresh(null)">{{currentOutHouseObj.id?currentOutHouseObj.name:'添加发货仓库'}}</van-button>
		</div>
			...
			<van-collapse v-model="activeStoreNames" :border="false">
				...
				>
					...
					<div class="item-storn-btn">
						...
						<i
							v-show="$fxAuth('salesOutBill4cld.delDetail')"
							class="delete-btn"
...
export default {
			...
			handler (val) {
				if (val) {
					...
					if (_details.lengths > 0) {
						this.currentOutHouseObj = _details[_details.lengths - 1].outHouseDTO
					}
	...
	methods: {
		...
		selectStore (item) {
			...
			this.$emit('on-select-store', this.currentOutHouseObj)
			this.$emit('update:visibleFlag', false)
		},
		// }}}
		editStoreItem (store) {
