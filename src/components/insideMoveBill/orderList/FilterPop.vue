	...
	<div class="filter-popup">
		...
		<div ref="filterContent" class="filter-content">
			<van-cell-group>
				...
				<van-cell title="出库仓库" :value="outHouse" value-class="info-item-value" is-link @click="onOutHouseClick" />
				<van-cell title="入库仓库" :value="store" value-class="info-item-value" is-link @click="onStoreClick" />
		...
		<van-popup v-model="outHousePopShow" position="right" class="fx-list-popup-300" :lock-scroll="false">
			<listComp
				:visibleFlag.sync="outHousePopShow"
				:data="outHouseList"
				:loadFinished="true"
				title="调出仓库检索"
				@on-search="searchOutHouse"
				@on-select="selectOutHouse"
			></listComp>
		</van-popup>
		<filterListPopup
			v-model="shopPopShow"
...
export default {
	...
	methods: {
		...
		searchOutHouse (keyword) {
			this.onOutHouseRefresh(keyword)
		},
		selectOutHouse (item) {
			this.outHouse = item.name
			this.outHouseId = item.id
		},
		onOutHouseRefresh (keyword = null) {
			this.getOutHouseListData(keyword).then(res => {
				this.outHouseList = res.shops
				this.outHousePopShow = true
			})
		},
		getOutHouseListData (keyword = null) {
			return new Promise(resolve => {
				let params = {
					m: 'getAllHouseList',
					keyword: keyword
