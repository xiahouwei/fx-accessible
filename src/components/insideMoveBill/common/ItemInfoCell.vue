		...
		<div class="item-details">
			...
			<div class="details-cell">
				<div class="item-taxprice fx-ellipsis">
					<span>非税金额：￥</span>
					<span>{{itemData.inMoney}}</span>
				</div>
				<div class="item-spec fx-ellipsis">{{itemData.goodsDTO.spec}}</div>
			</div>
			<div class="details-cell">
				<div v-if="isDisabled || (itemData.isWeightSign && !sysDZCCanModifyAcount) || !$fxAuth('insideMoveBill.editAmount')" class="item-amount">
				...
				<div v-else class="item-amount-edit">
					...
					<div class="unit-text">
						...
						<van-icon class="unit-icon" v-show="$fxAuth('insideMoveBill.modifyUnit') && !autoType8" name="arrow-down" />
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		onUnitClick () {
			if (this.isDisabled || !this.$fxAuth('insideMoveBill.modifyUnit') || this.autoType8) {
				return false
			}
		...
		onCheckAmount (tips = true) {
			return new Promise((resolve, reject) => {
				let { busUnitInAmount } = this.itemData
				let error = '数量不符合规则'
				if (busUnitInAmount === 0) {
					this.$nextTick(() => {
						if (tips) {
