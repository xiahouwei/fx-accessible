...
export default {
	...
	methods: {
		...
		getItemClass () {
			return this.$fxApi('distributionOutBill4cld.getItemClass', this.inShopId)
		},
		getItem () {
			const params = {
			...
			return this.$fxApi('distributionOutBill4cld.getItemByClass', this.inShopId)({ data: params })
		},
		getItemByClass () {
		...
		pushSelectedItem (item) {
			...
			if (!~index) {
				...
				if (!item.id && item.busUnitOutAmount === 0) {
					...
					if (item.id) {
						if (item.busUnitOutAmount === 0) {
							if (this.$fxAuth('distributionOutBill4cld.delDetail')) {
								this.$fxCommon.delEditItem(this.selectedItemArr, item, index)
							} else {
		...
		computedItemByAmount (item, index) {
			this.$fxCalculation('distributionOutBill4cld').changeAmount(this.selectedItemArr[index])
		},
		onKeyboardPre () {
			if (!this.rowActive) {
				return false
		...
		onUnitSelect (unit, item) {
			this.$fxCalculation('distributionOutBill4cld').changeUnit(this.itemList[this.rowActive], unit)
		},
		...
		getItemByCode (barcode) {
			...
			return this.$fxApi('distributionOutBill4cld.scanCode')({ data: params })
		},
		...
		copyScanGood (scanItem, selectGood) {
			...
			this.$fxCalculation('distributionOutBill4cld').changeAmount(_good)
			return _good
		},
		computedScanItemAmount (scanItem, selectGood) {
			...
			this.$fxCalculation('distributionOutBill4cld').changeAmount(scanItem)
		}
	}
}
</script>
