		...
		<div class="item-details">
			...
			<div v-show="itemData.assistUnit" class="item-details-cell">
				<div class='item-amount'>
					...
					<w-stepper
						v-show="!isDisabled"
						v-model="itemData.outAssistAmount"
						min="0"
						:max="maxAmount"
						:stepperBtnShow="false"
						:intLength="numberLength"
						...
						@blur="onBlur"
					></w-stepper>
					<span v-show="isDisabled">{{itemData.outAssistAmount}}</span>
				</div>
				<div class="unit-text">
...
export default {
	...
	methods: {
		...
		onBlur () {
			if (this.itemData.busUnitOutAmount === 0) {
				this.$fxToast.fail('数量不能为0!')
				this.itemData.busUnitOutAmount = 1
			}
			if (this.itemData.goodsDTO.assistUnitFlag && this.itemData.outAssistAmount === 0) {
				this.$fxToast.fail('辅助数量不能为0!')
				this.itemData.outAssistAmount = 1
			}
			this.$fxCalculation('distributionOutBill4cld').changeAmount(this.itemData)
