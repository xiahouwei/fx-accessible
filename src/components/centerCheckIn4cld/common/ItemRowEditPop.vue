<template>
	...
	>
		<van-cell-group>
			...
			<w-cell title="数量">{{itemData.busUnitInAmount}}</w-cell>
			<w-cell title="金额">{{itemData.inMoney}}</w-cell>
			<w-cell v-if="itemData.assistUnit" title="辅助单位">{{itemData.assistUnit}}</w-cell>
			<w-cell v-if="itemData.assistUnit" title="辅助数量">{{itemData.inAssistAmount}}</w-cell>
