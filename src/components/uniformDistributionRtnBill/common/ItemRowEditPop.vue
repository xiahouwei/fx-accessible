	...
	<div class="itemRowEdit-popup">
		...
		<div ref="rowEditContent" class="itemRowEdit-content">
			<van-cell-group>
				...
				<van-cell v-show="$fxAuth('uniformDistributionRtnBill.seePrice')" title="非税单价" :value="currentItemData.price" value-class="info-item-value"/>
				<van-cell v-show="$fxAuth('uniformDistributionRtnBill.noTaxOutmoney')" title="非税金额" :value="currentItemData.outMoney" value-class="info-item-value"/>
				<van-cell v-show="$fxAuth('uniformDistributionRtnBill.taxAmount')" title="税额" :value="currentItemData.taxOutMoney" value-class="info-item-value"/>
				<van-cell v-show="$fxAuth('uniformDistributionRtnBill.taxRete')" title="税率" :value="fomatterTaxes(currentItemData.tax)" value-class="info-item-value"/>
				...
				<van-cell v-show="sysShowDZCData && currentItemData.images.length" class="info-cell-img" value-class="info-item-img">
					<img :src="comptedItemImgSrc(currentItemData.images)" :onerror="$fxCommon.errorImg('currentItemData')">
				</van-cell>
			</van-cell-group>
		</div>
