	...
	<div class="order-block fx-shadow-block" @click="onClick">
		...
		<div class="block-content">
			...
			<div class="block-value">
				<div class="order-No">
					<span>{{data.undownBillDTOS.length ? data.undownBillDTOS[0].sysBillCode : ''}}</span>
				</div>
				<div class="order-date">
					<span>预估天数：</span>
					<span>{{data.reckonDateCount}}</span>
				</div>
			</div>
			<div class="block-value">
				<div class="order-No">
					<span>{{data.sysbillCode}}</span>
				</div>
				<div class="order-date">
					<span>创建时间：</span>
					<span>{{data.billDate | filterDateYMD}}</span>
