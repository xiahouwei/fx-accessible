	...
	<div class="store-item">
		...
		>
			<itemEditComp
				...
				:sysStockInSameGoods="sysStockInSameGoods"
...
import itemEditComp from '@/components/stockInBill/common/itemEditComp.vue'
...
export default {
	...
	methods: {
		...
		onItemEditConfirm ([selectedItemArr, store]) {
			...
			if (currentStore) {
				if (this.sysStockInSameGoods) {
					this.updateSameItem(currentStore, selectedItemArr)
				} else {
					this.updateItemByStore(currentStore, selectedItemArr)
				}
			} else {
		...
		updateSameItem (currentStore, selectedItemArr) {
			selectedItemArr.forEach(selectItem => {
				const indexs = this.$fxUtils.appointedMap(currentStore.details, (item, index) => {
					if (item.batchCode && selectItem.batchCode && item.batchCode === selectItem.batchCode && item.goodsDTO.id === selectItem.goodsDTO.id && item.changeType !== 'del') {
						return index
					}
				})
				if (indexs.length === 1) {
					if (currentStore.details[indexs[0]].id) {
						currentStore.details[indexs[0]].changeType = 'del'
					} else {
						currentStore.details.splice(indexs[0], 1)
					}
					currentStore.details.push(selectItem)
				} else {
					currentStore.details.push(selectItem)
				}
			})
		},
		updateItemByStore (currentStore, selectedItemArr) {
			selectedItemArr.forEach(selectItem => {
				const indexs = this.$fxUtils.appointedMap(currentStore.details, (item, index) => {
					if (item.goodsDTO.id === selectItem.goodsDTO.id && item.changeType !== 'del') {
						return index
					}
				})
				if (indexs.length === 1) {
