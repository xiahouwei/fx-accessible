	...
	<div class="filter-popup">
		...
		<div ref="filterContent" class="filter-content">
			<van-cell-group>
				...
				<van-cell title="出库机构" :value="shop" value-class="info-item-value" is-link @click="onShopClick" />
				...
				/>
				<van-cell title="预估报损" :value="esBreakName" value-class="info-item-value" is-link @click="onEsBreakClick" />
			</van-cell-group>
		</div>
		...
		<van-popup v-model="esBreakPopShow" position="right" class="fx-list-popup-300">
			<listComp
				:visibleFlag.sync="esBreakPopShow"
				:data="esBreakList"
				title="预估报损检索"
				:loadFinished="true"
				@on-select="selectEsBreak"
				@on-search="searchEsBreak"
			></listComp>
		</van-popup>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		// {{{ 报损预估
		onEsBreakClick () {
			this.esBreakList = this.esBreakData
			this.esBreakPopShow = true
		},
		selectEsBreak (item) {
			this.esBreakName = item.name
			this.esBreak = item.id
		},
		searchEsBreak (keyword) {
			this.esBreakList = this.esBreakData.filter(item => {
