<template>
	<div class="order-item-block fx-shadow-block">
		<div class="item-title">
			...
			<i v-show="!isDisabled && !isAutoType([3]) && $fxAuth('breakBill.bomBreak')" class="title-selectbom-icon title-icon fl-r" @click="onSelectBomClick"></i>
			<i v-show="$fxAuth('breakBill.bomBreak') && !isAutoType([3])" class="title-viewbom-icon title-icon fl-r" @click="onViewBomClick"></i>
			<i v-show="!isDisabled && !isAutoType([3])" class="title-add-icon title-icon fl-r" @click="onAddClick"></i>
...
<script>
import itemInfoCell from '@/components/breakBill/common/ItemInfoCell.vue'
export default {
	name: 'orderItem',
	components: {
		itemInfoCell
