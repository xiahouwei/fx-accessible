<template>
	<div class="store-item">
		...
		<div class="add-store-bar">
			<van-button v-if="currentOutHouseObj.id" class="add-store-btn" :disabled="!$fxSysAuth('distributionOutBill.sysMultiOutHouseForTPC')"  type="primary" round @click="onAddOutHouseClick">{{currentOutHouseObj.name}}</van-button>
			...
			<van-collapse v-model="activeStoreNames" :border="false">
				...
				>
					...
					<div class="item-storn-btn">
						...
						<i class="delete-btn" @click="deleteStoreItem(storeItem, storeIndex)"></i>
					</div>
					<div
...
export default {
		...
		visibleFlag: {
			handler (val) {
				if (val) {
					...
					if (hasDetails > 0 && !this.$fxSysAuth('distributionOutBill.sysMultiOutHouseForTPC')) { // 存在明细，且不允许多发货仓库
	...
	methods: {
		...
		// {{{ 发货仓库
		onAddOutHouseClick () {
			let hasStoreItem = this.currentStoreItemData.find(dataItem => {
				return dataItem.details.find(item => {
					return item.changeType !== 'del'
				})
			})
			if (hasStoreItem && !this.$fxSysAuth('distributionOutBill.sysMultiOutHouseForTPC')) {
				// 不允许多仓库，已有明细，不能修改发货仓库
				return false
			}
			if (this.autoType !== 1) {
				return false
		...
		// {{{ 收货仓库
		onAddShopHouseClick () {
			if (this.autoType === 1 || this.autoType === 6) {
