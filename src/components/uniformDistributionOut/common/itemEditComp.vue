...
export default {
	...
	methods: {
		...
		onItemConfirm () {
			...
			this.$nextTick(() => {
				this.selectedItemArr.forEach(item => {
					if (!item.shopHouseDTO) {
						item.shopHouseDTO = this.shopHouseDTO
					}
					if (!item.outHouseDTO) {
						item.outHouseDTO = this.outHouse
					}
				})
				this.$emit('on-confirm', [this.selectedItemArr, this.outHouse, this.shopHouseDTO])
				this.$emit('update:visibleFlag', false)
			})
		},
		search () {
		...
		getItemClass () {
			return new Promise(resolve => {
				this.$fxHttp.requestGet({
					url: `${this.$fxBaseApi.url}/accounts/${this.inShopId}/users/${this.loginerId}/goodTypes`
				}).then(res => {
					resolve(res)
				})
			})
		...
		getItem () {
			return new Promise(resolve => {
				let params = {
					...
					inHouseId: this.shopHouseDTO.id,
					workDate: this.businessDate,
					searchText: this.searchKey || ''
				}
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.inShopId}/users/${this.loginerId}/distributionOutDetails`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		setDataMoneyAndPrice (data) {
			data.forEach(item => {
				if (this.$fxUtils.isDef(item.pricePlanDTO)) { // 价格方案取固定价格
					item.price = this.toFixedNumberSize(item.pricePlanDTO.price)
					item.sendTaxes = this.toFixedNumberSize(item.pricePlanDTO.taxes)
					item.taxPrice = this.toFixedNumberSize(item.pricePlanDTO.price * (1 + item.pricePlanDTO.taxes))
					item.unitDTO = item.pricePlanDTO.unit
				} else {
					item.price = 0
					item.sendTaxes = 0
					item.taxPrice = 0
		...
		changeSelectedItem (item, index) {
			...
			this.calcCostMoney('amount', item.busUnitOutAmount, this.selectedItemArr[index])
		},
		// }}}
		onKeyboardPre () {
			if (!this.rowActive) {
		...
		getItemByRedCode (redCode) {
			// redCode = '202011201341000000020000201204114926314559820201204114900'
			// this.$fxToast.fail(redCode)
			// redCode = this.redCode
			if (!this.visibleFlag) {
				return false
			}
			if (redCode.length <= 24) {
				return this.onRedCodeHandler(redCode)
			} else { // 扫描二维码
				return this.onRedTwoCodeHandler(redCode)
			}
		},
		onRedCodeHandler (redCode) {
			// redCode = '0115000200001006000'
			this.rowActive = ''
			return this.getItemByRedCodeHandler(redCode).then(res => {
				let item = res.billAndDetails[0]
				item = this.itemUpdate(item)
				let amount = item.goodsDTO.countValue
				this.calcMoney('amount', amount, item)
				this.calcCostMoney('amount', amount, item)
		...
		itemUpdate (item) {
			...
			if (indexs.length === 1 && this.selectedItemArr[indexs[0]].id) {
				const storeItem = this.selectedItemArr[indexs[0]]
				const unitDTO = storeItem.unitDTO
				...
				storeItem.goodsDTO.countValue = countValue
				item = this.$fxUtils.JSONcopy(storeItem)
			}
			return item
		},
		getItemByRedCodeHandler (redCode) {
			return new Promise(resolve => {
				...
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.inShopId}/users/${this.loginerId}/distributionOutDetails/barCode/${redCode}`,
					data: params
				}).then(res => {
					resolve(res)
				})
		...
		onRedTwoCodeHandler (redCode) {
			return this.getItemByTwoCode(redCode, false).then(res => {
				if (res.errorCode === '100' || res.errorCode === '101') {
					return this.$dialog.confirm({
						...
						if (res.errorCode === '100') {
							return this.createBreakBill(res.storeBillDetailDTO)
						}
						return this.getItemByTwoCode(redCode, true).then(res => {
							return this.getScanGoodData(res.storeBillDetailDTO)
						})
					})
				} else {
					return this.getScanGoodData(res.storeBillDetailDTO)
		...
		getItemByTwoCode (barcode, nextFlag = false) {
			return new Promise(resolve => {
				let params = {
					...
					houseId: this.shopHouseDTO.id,
					outHouseId: this.outHouse.id,
					billType: 32,
					nextFlag: nextFlag
				}
				this.$fxHttp.requestPost({
					url: `${this.$fxBaseApi.url}/accounts/${this.inShopId}/users/${this.loginerId}/getscancodedata`,
					query: params
				}).then(res => {
					resolve(res)
				})
		...
		getScanGoodData (goods) {
			// 标记掌中宝扫码生成
			goods.autoType = 3
			goods.costPrice = this.toFixedNumberSize(goods.outMoney / goods.busUnitOutAmount)
		...
		setScanGood (scanGood) {
			...
			if (!~_index) {
				...
				if (!~_unitIndex) {
					...
					if (selectGood.batchCode !== scanGood.batchCode && this.$fxUtils.isDef(selectGood.batchCode) && selectGood.batchCode !== '') {
						this.$fxToast.fail('不能重复添加品项！')
						return false
					}
					let scanAmount = scanGood.busUnitOutAmount
					...
					this.$refs.scanActionSheet.doSelect().then(item => {
						...
						this.refreshSelectItem(scanGood, selectGood, scanAmount)
						this.itemList = [scanGood]
					})
				}
			}
		...
		refreshSelectItem (scanGood, selectGood, scanAmount) {
			...
			selectGood.costPrice = this.toFixedNumberSize(scanGood.outMoney / scanAmount)
			selectGood.outMoney = this.toFixedNumberSize(scanGood.busUnitOutAmount * selectGood.costPrice)
		},
		editItem (item) {
