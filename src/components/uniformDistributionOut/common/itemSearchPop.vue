			...
			<van-collapse v-model="activeStoreNames" :border="false">
				...
				>
					<div slot="title" class="item-store-title-container">
						<w-icon v-show="!isDisabled && !isAutoType(9)" type='camera' @click="onCameraClick(storeItem)"></w-icon>
						<div class="item-store-title">
							<span class="store-left-name fx-ellipsis">出库仓库：{{storeItem.name}}</span>
...
export default {
	...
	props: {
		...
		isDisabled: Boolean,
		visibleFlag: Boolean,
		autoType: { // 生成方式 0自动 1手动 5快速生成 9超收补单
			type: Number,
			default: 1
		}
	},
			...
			handler (val) {
				if (val) {
					...
					this.data.forEach(storeItem => {
						...
						if (hasDetails > 0) {
							this.activeStoreNames.push(`${storeItem.id}-${storeItem.shopHouseDTO.id}`)
						}
					})
				}
			},
	...
	methods: {
		...
		onUnitSelect (unit) {
			...
			if (amount > 0) {
				...
				this.calcCostMoney('taxPrice', item.taxPrice, item)
			}
			// 订货数量
		...
		onRedCodeHandler (redCode) {
			return this.getItemByCode(redCode).then(res => {
		...
		getItemByTwoCode (barcode, nextFlag = false) {
			return new Promise(resolve => {
				let params = {
					...
					houseId: this.scanStoreItem.shopHouseDTO.id,
		...
		setScanGood (scanGood) {
			...
			if (!~_index) {
				...
				if (!~_unitIndex) {
					...
					// 自动生成, 覆盖批次号
					if (this.isAutoType(0)) {
						this.$fxToast.top('扫码成功！')
		...
		refreshSelectItem (scanItem, selectGood, scanAmount) {
			...
			selectGood.costPrice = this.toFixedNumberSize(scanItem.outMoney / scanAmount)
			selectGood.outMoney = this.toFixedNumberSize(scanItem.busUnitOutAmount * selectGood.costPrice)
		},
		editItem (item) {
