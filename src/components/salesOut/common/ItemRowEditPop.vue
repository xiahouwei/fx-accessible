		...
		<div ref="rowEditContent" class="itemRowEdit-content">
			<van-cell-group>
				...
				<van-cell
					...
					value-class="info-item-value"
				></van-cell>
				<van-cell title="税率" :value="fomatterTaxes(currentItemData.sendTaxes)" :is-link="!isDisabled" value-class="info-item-value" :class="{ 'text-tax': !isDisabled }" @click="onTaxClick"/>
				<van-cell title="税额" :value="currentItemData.sendTaxMoney" value-class="info-item-value"/>
				<van-cell
...
export default {
	...
	methods: {
		...
		onTaxClick () {
			if (this.isDisabled || !this.$fxAuth('salesOutBill.editTax')) return false
