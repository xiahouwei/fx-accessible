	...
	<div class="filter-popup">
		...
		<div ref="filterContent" class="filter-content">
			<van-cell-group>
				...
				<van-cell title="采购机构" :value="shop" value-class="info-item-value" is-link @click="onShopClick" />
				<van-cell title="供货商" :value="supplier" value-class="info-item-value" is-link @click="onSupplierClick" />
				<van-cell title="采购类型" :value="stockTypeName" value-class="info-item-value" is-link @click="onStockTypeClick" />
		...
		<van-popup v-model="stockTypePopShow" position="right" class="fx-list-popup-300">
			<listComp
				:visibleFlag.sync="stockTypePopShow"
				:data="stockTypeList"
				title="采购类型检索"
				:loadFinished="true"
				@on-select="selectStockTypes"
				@on-search="searchStockTypes"
			></listComp>
		</van-popup>
...
export default {
	...
	methods: {
		...
		// {{{ 配送中心
		onStockTypeClick () {
			this.stockTypeList = this.stockTypeData
			this.stockTypePopShow = true
		},
		selectStockTypes (item) {
			this.stockTypeName = item.name
			this.stockType = item.id
		},
		searchStockTypes (keyword) {
			this.stockTypeList = this.stockTypeData.filter(item => {
