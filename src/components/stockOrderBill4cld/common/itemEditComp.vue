	...
	methods: {
		...
		getItem () {
			const params = {
				...
				houseId: this.inHouse.id,
				workDate: this.businessDate,
				searchText: this.searchKey || ''
			}
			return this.$fxApi('stockOrderBill4cld.getItemByClass', this.supplierId)({ data: params })
		},
		getItemByClass () {
		...
		pushSelectedItem (item) {
			...
			if (!~index) {
				...
				if (!item.id && item.busUnitInAmount === 0) {
					...
					if (item.id) {
						if (item.busUnitInAmount === 0) {
							if (this.$fxAuth('stockOrderBill4cld.delDetail')) {
								this.$fxCommon.delEditItem(this.selectedItemArr, item, index)
							} else {
		...
		computedItemByAmount (item, index) {
			this.$fxCalculation('stockOrderBill4cld').changeAmount(this.selectedItemArr[index])
		},
		onKeyboardPre () {
			if (!this.rowActive) {
				return false
		...
		onUnitSelect (unit, item) {
			this.$fxCalculation('stockOrderBill4cld').changeUnit(this.itemList[this.rowActive], unit, this.$fxAuth('stockOrderBill4cld.priceInTax') ? 'inTaxes' : 'outTaxes')
		},
