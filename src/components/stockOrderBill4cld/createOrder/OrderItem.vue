			...
			<van-collapse v-model="activeStoreNames" :border="false">
				...
				>
					<div slot="title" class="item-store-title-container">
						<span class="item-out-store-title fx-ellipsis">入库仓库：{{storeItem.name}}</span>
					</div>
					<van-swipe-cell
						...
						:key="item.uuid"
						:right-width="80"
						:disabled="isDisabled || focusing || !$fxAuth('stockOrderBill4cld.delDetail')"
						class="item-row"
					>
						<itemInfoCell
							v-bind="$attrs"
...
<script>
import itemInfoCell from '@/components/stockOrderBill4cld/common/ItemInfoCell'
export default {
	name: 'orderItem',
	components: {
		itemInfoCell
		...
		onUnitEdit (item) {
			...
			this.$fxApi('common.getItemUnit', item.goodsDTO.id).then(res => {
				...
				this.$refs.unitActionSheet.doSelect(item.unitDTO.id).then(unitItem => {
					this.$fxCalculation('stockOrderBill4cld').changeUnit(item, unitItem, this.$fxAuth('stockOrderBill4cld.priceInTax') ? 'inTaxes' : 'outTaxes')
				})
			})
		}
	}
